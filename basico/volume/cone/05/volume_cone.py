...
axL.set_ylim(-0.42, 1.18)
axL.axis("off")

axR = fig.add_axes([0.70, 0.14, 0.28, 0.76])
axR.set_xlim(0.0, r)

rho_plot = np.linspace(0.0, r, 400)
A_plot = A_of_rho(rho_plot)

axR.set_ylim(0.0, A_plot.max() * 1.20)
...
axL.text(0.0, -0.405, r"@Ivan Lopes",
         size=11, c=".2", alpha=0.35, ha="center", va="bottom")
...
(marker,) = axR.plot([0.0], [A_of_rho(0.0)], marker="o", markersize=7)
gtext = axR.text(0.02, 0.98, "", transform=axR.transAxes,
                 ha="left", va="top", fontsize=12)

# fill_between cria um PolyCollection (vamos atualizar com set_verts)
fill_coll = axR.fill_between(rho_plot, 0.0, 0.0 * rho_plot, alpha=0.20)


# ----------------------------
# Helpers geométricos (Paths)
# ----------------------------
def make_shell_path(rho, dr):
    rho_in = max(rho - dr, 0.0)
    rho_out = rho
...
# ----------------------------
# Preenchimento eficiente (SEM mexer em Path.vertices)
# ----------------------------
def make_fill_vertices(x, y, x_max):
    mask = (x <= x_max)
    xx = x[mask]
    yy = y[mask]

    if len(xx) < 2:
        # mínimo: triângulo degenerado válido
        x0 = float(x[0])
        return np.array([[x0, 0.0], [x0, 0.0], [x0, 0.0]], dtype=float)

    top = np.column_stack([xx, yy])
    bot = np.column_stack([xx[::-1], np.zeros_like(xx)])
...
def set_fill_upto(poly_collection, x_max):
    verts = make_fill_vertices(rho_plot, A_plot, x_max)
    # set_verts recebe lista de polígonos
...
def init():
    ...
    set_fill_upto(fill_coll, rho_plot[0])

    return (shell_patch, acc_patch, shell_top_line, marker, gtext, info_text, fill_coll)
...
def animate(i):
    ...
    info_text.set_text(
        ...
        r"$V_{\rm total}=\frac{1}{3}\pi r^2 h=" + f"{V_total:.4f}" + r"$"
    )

    marker.set_data([rho], [A_now])
    gtext.set_text(rf"$A(\rho)$ no ponto: {A_now:.4f}")

    set_fill_upto(fill_coll, rho)
