...
void update_window(const double         x,
                   const double         y,
                   const double         t,
                   ...
                   std::vector<double>& yt)
{
    ...
    const double half_win =
        (target_length / (2. * sqrt(1. + t * t)));
...
int main()
{
    ...
    for(size_t i = 0; i < n; i++)
    {
        ...
        {
            update_window(
                x[i], y[i], a * w * cos(w * x[i]), xt, yt);

            // Just update data for this plot.
            plot.update(xt, yt);

            // Small pause so the viewer has a chance to enjoy the
            // animation.
            plt::pause(0.1);
        }
    }
